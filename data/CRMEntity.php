...
//Added by Richie
	...
	*/
	...
	function transform_export_value($key, $value) {
		...
		$picklist_name =array(
			'accounttype','gl account','industry',
			'lead source','lead status','type',
			'product category','manufacturer',
			'rating','sales stage','salutation',
			'category','priority','severity',
			'status','usage unit','tracking unit','contract status',
			'contract priority','contract type','service usageunit',
			'service category',);

		if(in_array($key,$picklist_name))
			$value = (isset($mod_strings[$value])) ? $mod_strings[$value] : $value;
