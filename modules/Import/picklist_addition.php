...
foreach($field_to_pos as $fieldname => $ind)
{
	if(in_array($fieldname,$available_picklists))
	{
		...
		{
			if($current_mod_strings && $current_language != 'en_us')
			{
				$translate_picklist_key = array_search($datarows[$i][$ind],$current_mod_strings);
			}
			else
			{
				$translate_picklist_key = false;
			}
			$csv_picklist_values[$fieldname][] = ($translate_picklist_key) ? $translate_picklist_key : $datarows[$i][$ind];
		}
