...
function InsertImportRecords($rows,$rows1,$focus,$ret_field_count,$col_pos_to_field,$start,$recordcount,$module,$totalnoofrows,$skip_required_count)
{
	...
	//default picklist field name
	$picklist_name = array(
		'accounttype','activitytype','campaignstatus','campaigntype','carrier','eventstatus',
		'expectedresponse','faqcategories','faqstatus','glacct','industry','invoicestatus',
		'leadsource','leadstatus','manufacturer','opportunity_type','postatus','productcategory',
		'quotestage','rating','sales_stage','salutationtype','sostatus','taskpriority','taskstatus',
		'ticketcategories','ticketpriorities','ticketseverities','ticketstatus','usageunit',
		'tracking_unit','contract_status','contract_priority','contract_type','service_usageunit','servicecategory',
	);
	...
	$ii = $start;
	$current_mod_strings = return_specified_module_language($current_language,$_REQUEST['module']);
	...
	foreach ($rows1 as $row)
	{
		...
		for($field_count = 0; $field_count < $ret_field_count; $field_count++)
		{
			...
			if ( isset( $col_pos_to_field[$field_count]) )
			{
				...
				if(in_array($field,$picklist_name) && $current_language != 'en_us')
				{
					if($current_mod_strings)
					{
						$translate_picklist_key = array_search($row[$field_count],$current_mod_strings);
					}
					else
					{
						$translate_picklist_key = false;
					}
					if($translate_picklist_key)
					{
						$row[$field_count] = $translate_picklist_key;
					}
				}
