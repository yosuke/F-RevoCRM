...
$header->set_font(mb_convert_encoding('ＭＳ Ｐゴシック', "SJIS-win", "UTF-8"));


$val_font =& $workbook->addformat();
$val_font->set_font(mb_convert_encoding('ＭＳ Ｐゴシック', "SJIS-win", "UTF-8"));
...
if(isset($arr_val))
{
	...
	{
		$key = mb_convert_encoding($key, "SJIS-win", "UTF-8");
	...
	foreach($arr_val as $key=>$array_value)
	{
		...
		foreach($array_value as $hdr=>$value)
		{
			...
			$worksheet->write($key+1, $dcount, mb_convert_encoding($value, "SJIS-win", "UTF-8"),$val_font);//utf8_decode($value));
			$dcount = $dcount + 1;
		}
	}
