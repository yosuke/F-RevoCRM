	...
	function add_Activities($current_user,$free_busy='')
	{
		...
		if(!empty($activities))
		{
			...
			foreach($activities as $key=>$value)
			{
				...
				{
					if(!in_array($value->record,$tmp_record_ids)){
						$tmp_record_ids[] = $value->record;
						$date_formatt_str =  substr($value->formatted_datetime,0,10)." ".substr($value->formatted_datetime,-2).":00:00";
						try{
							$date = new DateTime($date_formatt_str);
						}catch(Exception $e) {
						}
						if($value->duration_hour >= 1){
							array_push($this->week_slice[$value->formatted_datetime]->activities, $value);
							for($i=1;$i<=$value->duration_hour;$i++){
								$date->modify("+1 hour");
								$new_formatted_datetime = $date->format('Y-m-d:H');
								if(!($i == $value->duration_hour && $value->end_time->minute == 0)){
									array_push($this->week_slice[$new_formatted_datetime]->activities, $value);
								}
							}	
						}else{
							array_push($this->week_slice[$value->formatted_datetime]->activities, $value);
