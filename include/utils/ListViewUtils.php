...
function getPopupCheckquery($current_module,$relmodule,$relmod_recordid)
{
	...
	if($current_module == "Contacts")	
	{
		...
		elseif($relmodule == "Potentials")
		{
			...
			$rows = $adb->getRowCount($result);
			if($rows >0)
			{
				$contact_ids = array();
				for($i=0;$i<$rows;$i++){
					$contact_ids[] = $adb->query_result($result,$i,"contactid");
				}
				$contact_id = implode(",",$contact_ids);
				$condition = "and vtiger_contactdetails.contactid in ( ". $contact_id .") ";
			}
			else
			{
