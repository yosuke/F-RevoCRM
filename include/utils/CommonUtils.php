...
/**
 * Function to get the reports_to_id when a vtiger_users id is given
 * Takes the input as $login_user_id - vtiger_users id
 * returns the Users id.
 */

function getReportsToId($login_user_id)
{
        global $log;
        $log->debug("Entering getReportsToId(".$login_user_id.") method ...");
        $log->info("in getReportsToId ".$login_user_id);
        global $adb;
        $sql = "select * from vtiger_users where id=?";
        $result = $adb->pquery($sql, array($login_user_id));
        $reports_to_id = $adb->query_result($result,0,"reports_to_id");
        $log->debug("Exiting getReportsToId method ...");
